...
public class CnosDBOptions implements DBMSSpecificOptions<CnosDBOracleFactory> {
    ...
    public List<CnosDBOracleFactory> oracle = Arrays.asList(CnosDBOracleFactory.QUERY_PARTITIONING);

    @Parameter(names = "--connection-url", description = "Specifies the URL for connecting to the CnosDB", arity = 1)
    public String connectionURL = String.format("http://%s:%d", CnosDBOptions.DEFAULT_HOST,
            CnosDBOptions.DEFAULT_PORT);
